local mod = class("graphic",cls.mod.base)
mod.heat_produce = 0
mod.energy_occupy = 0
mod.mod_name = "视图"
mod.socket = "system"
local root = "module/system/hud/"
...
function mod:init(ship,slot)
	mod.super.init(self,ship,slot)	
	self.cam = Camera.new(-5000,-5000,10000,10000)
	self.grid = require("lib/grid").new(self.cam)
	self.zoom = 1
	self.energy_spot_mesh = circle(20,0)
    self.ui = require(root.."ui"):init(self)
	self.fadeout = require(root.."fadeout"):init()
	self.explosion = require(root.."explosion"):init()
    self.bloom = require(root.."bloom")(400,300)
	game.hud = self
...
function mod:drawModRange()
    local show = self.ship.data.showRange
    if not show then return end
    
    self.cam:draw(function()
        if show.w then
            love.graphics.setColor(100, 100, 255, 150)
            local sx,sy = self.ship.x+math.sin(self.ship.angle)*show.min_r/self.cam.scale,
                        self.ship.y-math.cos(self.ship.angle)*show.min_r/self.cam.scale
            local tx,ty = self.ship.x+math.sin(self.ship.angle)*show.max_r/self.cam.scale,
                        self.ship.y-math.cos(self.ship.angle)*show.max_r/self.cam.scale
            love.graphics.setLineWidth(show.w/self.cam.scale)
            love.graphics.line(sx,sy,tx,ty)
            love.graphics.circle("fill",sx,sy,show.w/self.cam.scale/2)
            love.graphics.circle("fill",tx,ty,show.w/self.cam.scale/2)
            love.graphics.setLineWidth(1)
        else
            love.graphics.push()
            love.graphics.translate(self.ship.x,self.ship.y)
            love.graphics.rotate(self.ship.angle)
                local function to_cut()
                    love.graphics.arc("fill", 0, 0, show.min_r/self.cam.scale, 
                        -Pi/2 - Pi*show.percent , Pi*show.percent -Pi/2)
                end
                ...
                love.graphics.setColor(100, 100, 255, 150)
                love.graphics.arc("fill", 0, 0, show.max_r/self.cam.scale, 
                        -Pi/2 - Pi*show.percent , Pi*show.percent - Pi/2)
                love.graphics.setStencilTest()
            love.graphics.pop()
        end
        if show.target_w then
            
            love.graphics.setColor(200, 100, 255, 150)
            local sx,sy = self.ship.x,self.ship.y
            local tx,ty = self.cam:toWorld(love.mouse.getPosition())
            love.graphics.setLineWidth(show.target_w/self.cam.scale)
            love.graphics.line(sx,sy,tx,ty)
            love.graphics.circle("fill",sx,sy,show.target_w/self.cam.scale/2)
            love.graphics.circle("fill",tx,ty,show.target_w/self.cam.scale/2)
            love.graphics.setLineWidth(1)
        elseif show.target_r then
            love.graphics.push()
            love.graphics.translate(self.cam:toWorld(love.mouse.getPosition()))
            love.graphics.setColor(200, 100, 255, 150)
            love.graphics.circle("fill",0,0,show.target_r/self.cam.scale)
            love.graphics.pop()
        else
            
            love.graphics.push()
            love.graphics.translate(self.cam:toWorld(love.mouse.getPosition()))
            love.graphics.setColor(200, 100, 155, 150)
            love.graphics.setLineWidth(1)
